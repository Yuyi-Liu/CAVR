...
from rearrange_on_proc.constants import ALL_CATEGORY_LIST
...
class RearrangementTask_AI2THOR():
    def __init__(self, task_id,controller, task,process_id, task_index, solution_config, gpu_id = 0,online_controller = None) -> None:
    ...
    def fix_task_data(self):
        start_poses = self.curr_task["starting_poses"]
        target_poses = self.curr_task["target_poses"]
        position_diff_num = self.curr_task["position_diff_count"]
        diff_poses = {}
        for i,obj in enumerate(start_poses):
            
            start_position = obj["position"]
            start_rotation = obj["rotation"]
            target_obj = next(x for x in target_poses if x["name"] == obj["name"])
            
            target_position = target_obj["position"]
            target_rotation = target_obj["rotation"]
            position_diff = abs(start_position["x"]-target_position["x"])+abs(start_position["y"]-target_position["y"])+abs(start_position["z"]-target_position["z"])
            rotation_diff = abs(start_rotation["x"]-target_rotation["x"])+abs(start_rotation["y"]-target_rotation["y"])+abs(start_rotation["z"]-target_rotation["z"])
            pose_diff = position_diff + rotation_diff
            if pose_diff > 0.01:
                diff_poses[obj["name"]] = position_diff
        sorted_items = sorted(diff_poses, key=lambda x: diff_poses[x], reverse=True)
        sorted_items = sorted_items[position_diff_num:]
        for obj_id in sorted_items:
            start_index = next(i for i in range(len(start_poses)) if start_poses[i]["name"] == obj_id)
            target_index = next(i for i in range(len(target_poses)) if start_poses[i]["name"] == obj_id)
            self.curr_task["starting_poses"][start_index] = self.curr_task["target_poses"][target_index]
        


    def set_walkthrough_or_unshuffle_env(self, task_index, stage, curr_scene_unique_id,walkthrough_object_metadata=None):
        # for obj_type in ALL_CATEGORY_LIST:
        #     self.controller.step(action='MakeObjectsOfTypeUnbreakable', objectType=obj_type)
        if stage == 'walkthrough':
            
            self.controller.reset(scene = self.curr_scene_id)
            print(f'====For Process {self.process_id}-task {task_index}: house {self.curr_scene_unique_id}====')
            assert self.controller.last_event.metadata['lastActionSuccess'] == True, f"Reset [Env] Fails!!!"
            
            args.map_size = 10 * 2    #10 * 2
            args.step_max = 300
            args.rearrange_step_max = 80

        
        print(f"---:p{self.process_id}:{self.task_id} ---「{stage}」---")
        ...
        if stage == 'walkthrough':
            #for pipline
            curr_stage_pickupable_objects_poses = []
            for obj in self.curr_task['target_poses']:
                if "SaltShaker" in obj["name"]:
                    continue
                curr_stage_pickupable_objects_poses.append(obj)
            # curr_stage_pickupable_objects_poses = self.curr_task['target_poses']
        elif stage == 'unshuffle':
            curr_stage_pickupable_objects_poses = []
            for obj in self.curr_task['starting_poses']:
                if "SaltShaker" in obj["name"]:
                    continue
                curr_stage_pickupable_objects_poses.append(obj)
        ...
        #     obj = next(obj for obj in walkthrough_object_metadata if "Television" in obj["name"])
        #     kept_object_poses.append({
        #                 "objectName": obj['name'],
        #                 "position": obj['position'],
        #                 "rotation": obj['rotation']})
        #     for l_obj in walkthrough_object_metadata:
    ...
    def main(self, ):
        ...
        print(f"========Process {self.process_id}: {self.task_id} task=========")

        scene_id, reuse_id = self.task_id.split('_')
        ...
        self.curr_scene_unique_id = self.task_id
        ...
        # with open("metrics/metrics_GTtwo_phase|cover_continue|same|feature_pointcloud_based|greedy.json","r") as f:
        #     metrics = json.load(f)
        # from PIL import Image
        # curr_metirc = metrics["FloorPlan10_36"]
        # unshuffle_actions = curr_metirc["unshuffle_actions"]
        # pick_num = 0
        # for i in range(len(unshuffle_actions)):
        #     act = unshuffle_actions[i]
        #     Image.fromarray(self.controller.last_event.frame).save(f"supplementary/rearrange_img/{i}.jpg")
        #     if "Pick" in act:
        #         pick_up_id = curr_metirc["pickup_obj_ids"][pick_num]
        #         pick_num+=1
        #         self.controller.step("")

        self.curr_stage = 'walkthrough'
        self.set_walkthrough_or_unshuffle_env(self.task_index, self.curr_stage, self.curr_scene_unique_id)
        walkthrough_object_metadata = self.controller.last_event.metadata["objects"]

        if args.test_mode == "only_walkthrough":
            args.step_max = args.only_walkthrough_mode_max_steps
            args.movie_dir = f"./movies_{args.seg_utils}{args.test_mode}|{args.walkthrough_search}|{args.only_walkthrough_mode_max_steps}"
        elif args.test_mode == 'only_walkthrough_steps':
            args.step_max = 1000
            args.movie_dir = f"./movies_{args.seg_utils}{args.test_mode}|{args.walkthrough_search}|{args.only_walkthrough_mode_max_steps}"
        else:
            f"./movies_{args.seg_utils}{args.test_mode}|{args.walkthrough_search}|{args.unshuffle_search}|{args.unshuffle_match}|{args.unshuffle_reorder}"
        walkthrough_explorer = Explore(controller=self.controller, gpu_id=self.gpu_id,vis=vis, visdom=visdom, curr_stage = self.curr_stage, map_size=args.map_size, step_max = args.step_max, process_id = self.process_id,solution_config=self.solution_config)
        ...
        self.curr_stage = 'unshuffle'
        self.set_walkthrough_or_unshuffle_env(self.task_index, self.curr_stage, self.curr_scene_unique_id,walkthrough_object_metadata)
        walkthrough_rgbs = walkthrough_explorer.walkthrough_rgb_list
        walkthrough_pointclouds = walkthrough_explorer.walkthrough_point_cloud_list
        walkthrough_pointclouds_isnan = walkthrough_explorer.walkthrough_point_cloud_isnan_list
        walkthrough_actions = walkthrough_explorer.walkthrough_action_list
        walkthrough_depths = walkthrough_explorer.walkthrough_depth_list
        unshuffle_explorer = Explore(controller=self.controller,
                                     gpu_id = self.gpu_id,
                                     vis=vis, 
                                     visdom=visdom,  
                                     ...
                                     walkthrough_rgbs=walkthrough_rgbs,
                                     walkthrough_pointclouds = walkthrough_pointclouds,
                                     walkthrough_pointclouds_isnan = walkthrough_pointclouds_isnan,
                                     walkthrough_actions = walkthrough_actions,
                                     walkthrough_depths = walkthrough_depths,
                                     walkthrough_objs_id_to_pose=walkthrough_objs_id_to_pose_init,
                                     walkthrough_explorer=walkthrough_explorer)
        ...
        rearranger = Rearrange(task_id = self.task_id,
                               walkthrouth_explorer=walkthrough_explorer,
                               unshuffle_explorer=unshuffle_explorer, 
                               process_id = self.process_id, 
                               task=self.curr_task, 
                               visdom=visdom, 
                               walkthrough_objs_id_to_pose= walkthrough_objs_id_to_pose_init,
                               unshuffle_objs_id_to_pose=self.unshuffle_objs_id_to_pose,
                               solution_config=self.solution_config,
                               gpu_id=self.gpu_id)
