...
from rearrange_on_proc.datagen.datagen_constants import OBJECT_TYPES_TO_NOT_MOVE, IOU_THRESHOLD, OPENNESS_THRESHOLD, POSITION_DIFF_BARRIER
from rearrange_on_proc.datagen.datagen_utils import (
    open_objs,
    get_object_ids_to_not_move_from_object_types,
    remove_objects_until_all_have_identical_meshes,
)
from rearrange_on_proc.rearrange_challenge.constants import (
...
# from rearrange_on_proc.rearrange_challenge.arguments import args
from rearrange_on_proc.datagen.datagen_utils import get_top_down_frame
# lwj test procTHOR
# import prior
# dataset = prior.load_dataset("procthor-10k")
# houseId = 1
# house = dataset['train'][houseId]
# roomSpecId = house["metadata"]['roomSpecId']
...
class RearrangeTHOREnvironment:
    ...
    obj_id_to_walkthrough_start_pose : Dictionary mapping AI2-THOR object id to their poses (positions & rotations)
    ...
    obj_id_to_unshuffle_start_pose : Same as `obj_id_to_walkthrough_start_pose` but mapping object names to their poses (positions &
        ...
        if self._sorted_and_extracted_walkthrough_start_poses is None:
            ...
            if len(broken_obj_ids) != 0:
                if not self.current_task_spec.runtime_sample:
                    ...
                    # )
                    print(f"BROKEN GOAL OBJECT!{broken_obj_ids} ")
    ...
    def _task_spec_reset(
        self, task_spec: RearrangeTaskSpec, force_axis_aligned_start: bool,  modify_objs = {},
    ):
        ...
        # self.controller.reset(house)
        # return
        # print("OBJ_reset:", [l_obj["name"] for l_obj in self.last_event.metadata["objects"]])
        ...
        with include_object_data(self.controller):
            ...
            # print("OBJ_reset_open:", [l_obj["name"] for l_obj in self.last_event.metadata["objects"]])
            # print('GarbageCan!!! ')
            # for l_obj in self.last_event.metadata["objects"]:
            #     if 'GarbageCan' in l_obj['name']:
            #         print(l_obj)

            considered_pickupable_object_names =  [obj['name'] for obj in self.current_task_spec.target_poses]
            kept_object_poses = []
            for l_obj in self.last_event.metadata["objects"]:
            ...
            self.controller.step(
                "SetObjectPoses",
                objectPoses=self.current_task_spec.target_poses + kept_object_poses,
            ...
            # img1 = get_top_down_frame(self.controller)
            # import os
            # img1.save(os.path.join(STARTER_DATA_DIR, f'test18_reset.png'))

    def reset(
        self, task_spec: RearrangeTaskSpec, force_axis_aligned_start: bool = False, modify_objs = {}
    ) -> None:
        ...
        if task_spec.runtime_sample:
            ...
            self._task_spec_reset(
                task_spec=task_spec, force_axis_aligned_start=force_axis_aligned_start, modify_objs = modify_objs
            )
    ...
    def _reset_with_modify(self):
        if len(self.modify_objs):
            objId_to_moveable = {}
            for obj in self.controller.last_event.metadata['objects']:
                objId_to_moveable[obj['objectId']] = obj['moveable']

            objposes = []
            ifReceptacle_recepObjs = []
            for obj in self.controller.last_event.metadata['objects']:
                if (obj['moveable'] or obj['pickupable']):
                    pos = obj['position']
                    rot = obj['rotation']

                    obj_operations = self.modify_objs.get(obj['objectId'], None)
                    if obj_operations is not None:
                        op = list(obj_operations.keys())[0]
                        info = obj_operations[op]
                        if op == 'ChangePosition':
                            pos[info[0]] += info[1]
                        elif op == 'ChangeRotation':
                            rot[info[0]] += info[1]
                        elif op == 'ObstacleRemove' or op == 'ObjRemove':
                            if obj['receptacle']:
                                for recepObj in obj['receptacleObjectIds']:
                                    if objId_to_moveable[recepObj]:
                                        ifReceptacle_recepObjs.append(recepObj)
                            continue
            ...
            n_objposes_bofore = len(objposes)
            for i in range(len(objposes)-1, -1, -1):
                if objposes[i]['objectId'] in ifReceptacle_recepObjs:
                    objposes.pop(i)
            if n_objposes_bofore != len(objposes):
                print(f"Filter moveable objs on ObstacleRemove {n_objposes_bofore} -> {len(objposes)} ")
            ...
            # img1 = get_top_down_frame(self.controller)
            # import os
            # img1.save(os.path.join(STARTER_DATA_DIR, f'test_modify.png'))
            print('Modify Objs For reset (environment task_spec_reset)', self.controller.last_event.metadata['errorMessage'])
    ...
    def _task_spec_shuffle(self, reset: bool = False):
        ...
        # lwj test procTHOR
        # return 
        # print("OBJ_shuffle0:", [l_obj["name"] for l_obj in self.last_event.metadata["objects"]])
        ...
        # print("OBJ_shuffle:", [l_obj["name"] for l_obj in self.last_event.metadata["objects"]])
        ...
        with include_object_data(self.controller):
            for obj in task_spec.openable_data:
                ...
                current_obj_info = next(l_obj for l_obj in self.last_event.metadata["objects"]
                    if l_obj["objectId"] == obj["objectId"]
                )
        ...
        considered_pickupable_object_names =  [obj['name'] for obj in task_spec.starting_poses]
        kept_object_poses = []
        for l_obj in self.last_event.metadata["objects"]:
            if l_obj['name'] not in considered_pickupable_object_names and l_obj['objectType'] not in OBJECT_TYPES_TO_NOT_MOVE:
                kept_object_poses.append({
        ...
        self.controller.step(
            "SetObjectPoses",
            objectPoses=task_spec.starting_poses + kept_object_poses,
            # placeStationary=False,
            placeStationary=True,
            enablePhysicsJitter=True,
            forceRigidbodySleep=True,
        ...
        # img1 = get_top_down_frame(self.controller)
        # import os
        # img1.save(os.path.join(STARTER_DATA_DIR, f'test18_shuffle.png'))
    ...
    @staticmethod
    def _obj_list_to_obj_id_to_pose_dict(
